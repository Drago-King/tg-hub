...
async def get_tool_schema(tool_name: str, timeout: float = 30.0) -> dict:
    """Returns the full input schema for one tool — use this to confirm
    exact parameter names before calling a tool blind."""
    async with httpx.AsyncClient(timeout=timeout) as client:
        session_id = await _ensure_session(client)
        body = {"jsonrpc": "2.0", "id": str(uuid.uuid4()), "method": "tools/list"}
        resp = await client.post(SWIGGY_MCP_URL, json=body, headers=_headers(session_id))
